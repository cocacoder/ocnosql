...
//    final static String COLUMNS_CONF_KEY = "importtsv.columns";
    ...
    public static Job createSubmittableJob(Configuration conf, String[] args)
        ...
        TableMapReduceUtil.addDependencyJars(job.getConfiguration(),com.ailk.oci.ocnosql.common.OCNosqlNestedRuntimeException.class);
        TableMapReduceUtil.addDependencyJars(job.getConfiguration(),org.springframework.core.NestedRuntimeException.class);
        TableMapReduceUtil.addDependencyJars(job.getConfiguration(),org.apache.commons.lang3.StringUtils.class);
        String hfileOutPath = conf.get(BULK_OUTPUT_CONF_KEY);
        if (hfileOutPath != null) {
            List<HTable> tables = new ArrayList<HTable>();
            for (byte[] tableName : new HbasePutUtil().retrieveTablesName()) {
                tables.add(new HTable(conf, tableName));
                HFileOutputFormat.configureIncrementalLoad(job, tables);
            }
            job.setReducerClass(PutSortReducer.class);
            Path outputDir = new Path(hfileOutPath);
            FileOutputFormat.setOutputPath(job, outputDir);
            job.setMapOutputKeyClass(ImmutableBytesPairWritable.class);
            job.setMapOutputValueClass(Put.class);
        } else {
            // No reducers.  Just write straight to table.  Call initTableReducerJob
            // to set up the TableOutputFormat.
//            TableMapReduceUtil.initTableReducerJob(tableNames, null, job);
